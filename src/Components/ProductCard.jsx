    ...
    onClick={()=>{navigate(`/product/${product.id}`)}}
     className=" w-full group bg-white border border-gray-200 rounded-xl pb-2 hover:-translate-y-1 transition-all duration-300 hover:shadow-lg cursor-pointer overflow-hidden">
      ...
      <div className="px-3 py-4 sm:px-4 sm:py-5">
        <h3 className="text-lg sm:text-xl font-semibold line-clamp-1">{product.title.split(" ").slice(0,3).join(" ")}</h3>
        <p className="text-sm text-gray-500 mt-1 line-clamp-1 h-[70px]">
          {/* {product.description} */}
        ...
        <div className="flex justify-between items-center mt-4">
          <div className="text-green-500 text-base sm:text-xl flex gap-1.5 items-center">
            <RiMoneyPoundCircleLine size={22} />
            ${product.price}
          </div>
          <div className="rounded-2xl border border-gray-400 flex justify-center items-center px-2 py-0.5 text-xs font-semibold">
            {product.category}
          </div>
        </div>
      </div>
